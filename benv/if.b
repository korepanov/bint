...
void replace_elseif(string cond, int stop_pos){
	...
	[print(""), (len(command) > 6), goto(#replace_else_ret_end2)];
	print("");
	
	[print(""), (command[0:6] == "return"), goto(#replace_else_ret_end2)];
	print("");
	[print(""), (COMMAND_COUNTER < stop_pos), goto(#replace_else_ret_end2)]; 
	
	args_to_undefine_old = args_to_undefine;

	args_to_undefine.pop(arg_name);
	#un9:
	[goto(#un_end9), ("end" == arg_name), print("")];
	buf = (("UNDEFINE(" + arg_name) + ")");
	send_command(buf);
	args_to_undefine.pop(arg_name); 
	goto(#un9);
