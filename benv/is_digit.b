...
void init(){
	...
	SET_DEST("benv/is_digit_program.b");	
};
...
stack is_digit_poses(string command){
	stack s;
	stack el;
	stack res;
	stack null; 
...
void modify(){
	...
	while (NOT("end" == command)){
		...
		while (NOT("end" == buf)){
			...
			epos = (epos + 1);
			snumber = str(number);
			buf = ("bool $dig" + snumber);
			send_command(buf);
			buf = command[bpos:epos];
			buf = ((("$dig" + snumber) + "=") + buf);
			send_command(buf);
			sub_command = ("$dig" + snumber);
...
void main(){
	...
	if ("benv/import_program.b" == root_source){
		copy("benv/is_digit_program.b", "benv/import_program.b");
	}else{
		copy("benv/is_digit_program.b", "benv/trace_program.b");
	};
	
	DEL_DEST("benv/is_digit_program.b");
	...
	for (int number; number = 0; exists((("benv/trace/trace_program" + str(number)) + ".b")); number = (number + 1)){
		...
		SET_DEST((("benv/trace/is_digit_program" + str(number)) + ".b"));
		modify();
		finish();
		copy((("benv/trace/is_digit_program" + str(number)) + ".b"), (("benv/trace/trace_program" + str(number)) + ".b"));
		DEL_DEST((("benv/trace/is_digit_program" + str(number)) + ".b"));
	};
};
