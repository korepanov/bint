...
#next:print("");
switch_command();
if(NOT("end"==command)){sold_num=str(old_num);
buf2=("#_for"+sold_num);
buf_len=len(buf2);
if(len(command)>buf_len){if(command[0:buf_len]==buf2){if(was_internal_for){was_shift=True;
...
print("");
if(is_for(command)){was_for=True;
was_internal_for=False;
command_len=len(command);
command=command[4:command_len];
if(NOT(was_shift)){send_command(command);
};
print("");
old_command=command;
switch_command();
...
#next_internal:print("");
switch_command();
if(COMMAND_COUNTER<counter){if(is_for(command)){command_len2=len(command);
