# Интерпретатор с шифрованием для языков B и Basm
...
main();
```
## Разложение на элементарные операции и шифрование 
Разложение программые на языке **Basm** на элементарные операции заключается в представлении
исходной программы на языке (**Bend**), который может быть интерпретирован минимальным ядром интерпретатора 
Шифрование заключается в добавлении мусора (реально существующих случайных команд) внутрь конструкций 
языка **Bend** и запоминании смещений, по которым лежат реальные команды, в отдельный файл с раширением **k**.
Полученный зашифрованный файл имеет расширение **benc** и может быть проинтерпетирован с файлом-ключом **k**
## Начать работу 
...
```
### Интерпретировать программу на языке Bend 
```
./bint -pe output.bend 
```
### Зашифровать программу на языке Bend, сгенерировав файл с ключом key.k
```
./bint -ci input.bend -co output.benc -k key.k
```
### Выполнить зашифрованную программу input.benc с ключом key.k
```
