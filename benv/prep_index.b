...
void init(){
	...
	SET_DEST("benv/prep_index_program.b");	
};
...
void main(){
	...
	if ("benv/import_program.b" == root_source){
		copy("benv/prep_index_program.b", "benv/import_program.b");
	}else{
		copy("benv/prep_index_program.b", "benv/trace_program.b");
	};
	
	DEL_DEST("benv/prep_index_program.b");
	string t;
	string t2;
	string s;
	s = str(0);
	...
	for (int number; number = 1; e; number = (number + 1)){
		t = (("benv/trace/trace_program" + s) + ".b");
		SET_SOURCE(t);
		t = (("benv/trace/prep_index_program" + s) + ".b");
		SET_DEST(t);
		modify();
		finish();
		t = (("benv/trace/prep_index_program" + s) + ".b");
